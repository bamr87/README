...
For a **muse** (short-form opinion/news), the author sets `section_guide: devops-news-muse` → profile `muse-opinion`.
...
these are the rules you apply to the prose (DRY → link don't restate; DFF → be honest about failure; KIS → plain words; AIPD → transparent about AI).
...
relaxes them in `.cms/config.yml › brand.sections.<slug>.banned_terms_relax`); use canonical spellings (GitHub, JavaScript, CI/CD, VS Code, Node.js).
...
code) plus the section's required elements — e.g. `devops` and `system-administration` posts must show a **Verify** step.
...
If you can, run `make cms-all` (or `python3 scripts/cms/cms.py analyze`) and read the **Brand drift by section** table in `.cms/reports/<date>.md` to confirm the piece is clean. (The `/brand-audit` prompt does exactly this.)

## 5. Hand off

Return the draft or the revised prose. State which section + profile you resolved, and note anything you deliberately deviated from (and why). Do not commit, push, or open a PR.
...
voice; **`/draft-muse`** is the short-form sibling; **`/brand-audit`** reports drift. This skill is the shared brand-context loader underneath all three.
