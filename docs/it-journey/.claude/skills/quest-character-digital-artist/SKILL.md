---
description: Play or repair IT-Journey quests as the 🎨 Digital Artist (UI/UX) character
  path — persona, voice, per-level exercise checkpoints, and friction lens for digital-artist/<level>
  slices. Load (Read this file) whenever walk-plan.json or a fix slice names character
  "digital-artist", when walking quests as this class, or when judging whether a quest
  serves this path's learner.
