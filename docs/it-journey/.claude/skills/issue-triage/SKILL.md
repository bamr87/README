---
description: Run ONE incremental pass of the IT-Journey issue autopilot — analyze
  open issues, group them into PR-sized batches, then either route/triage them (close
  stale bot-noise, decompose epics, flag for a human) or resolve one batch into a
  single grouped PR. Use when asked to triage issues, work the issue queue, run the
  issue-autopilot loop, or drive the issue-triager / issue-resolver agents. Mirrors
  the cms-curator loop idiom.
name: issue-triage
source_file: SKILL.md
title: Issue Triage — one incremental pass of the issue autopilot
---
