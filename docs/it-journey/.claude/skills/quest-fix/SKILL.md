...
description: Consume ONE walkthrough's verified evidence and apply the smallest content-only
  ...
  it breaks; this skill repairs exactly what it witnessed, proving each edit helped
  with a deterministic signal (never the model's own grade). Use when asked to "fix
  a walked quest", run the quest fix lane, or driven by the fix arm of the quest-perfection
  loop. Writes only quest content; never branches, commits, or merges.
name: quest-fix
