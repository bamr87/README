...
| `content-auto-merge.yml` | PR events; dispatch with a `pr` input (gated) | **The single label-routed auto-merge lane** (absorbed `issue-pr-auto-merge.yml` and `quest-report-auto-merge.yml`). Routes by label — `auto:content` (content-only diff, `CONTENT_AUTOMERGE_ENABLED`), `auto:issue` (same-repo + resolver scope, `ISSUE_AUTOMERGE_ENABLED`; merging closes the linked issues), `automated`+`quest-walkthrough` (report/ledger paths, `QUEST_REPORT_AUTOMERGE_ENABLED`) — then smuggle-guard (`scripts/ci/classify_changes.py`) + all checks green → squash-merge. Labels are read **live from the API**, never from the event payload: the payload is a snapshot, `opened` carries none, and delivery order is not guaranteed, so snapshot-gating silently dropped PRs (#563). Hands off to `pr-freshness.yml` after a merge. |
| `pr-freshness.yml` | push to `main`; after a merge; 6-hourly; dispatch (gated) | **Keeps the loops' open PRs mergeable.** For every open `automated`/`auto:*` PR in this repo: `BEHIND` → merge the base in; `DIRTY` report PR → `scripts/quest/refresh_report_branch.sh` rebuilds its ledger + dashboard + collection from their inputs; otherwise re-dispatch the merge lane in case its PR-event wake-up was lost. Conflicts outside the regenerated set get `needs-human`, never an automatic resolution. OFF behind `PR_FRESHNESS_ENABLED`. |
...
| `quest-perfection.yml` | daily; dispatch (gated) | **The WALK (measurement) lane of the quest-perfection loop.** Selects THE single highest-priority not-yet-perfect (character, level) slice from the committed ledger (`scripts/quest/ledger.py select`), drives the walk, and updates `.quests/ledger.json` + the generated `.quests/DASHBOARD.md`. Rotation covers the other paths over following days (the ledger's selection key sorts by `last_run` ascending); `all_paths: true` on a dispatch restores the full one-slice-per-character sweep (~6× the cost). Uploads each walk's sealed plan+evidence as `quest-perfection-<char>-<code>` — the durable handoff the fix lane reads. OFF behind `QUEST_PERFECTION_ENABLED`. |
| `quest-video.yml` | dispatch (gated) | **The VIDEO lane.** Records a side-by-side walkthrough video of a slice's main quest (rendered quest page ⇄ animated terminal replay of the sealed execute-engine transcript, via `scripts/quest/walkthrough_video.mjs`) — evidence reused from a `quest-perfection-*`/`quest-walkthrough-*` artifact (`source_run_id`) or minted by a fresh deterministic engine pass. Always uploads the video as a 14-day run artifact; with `publish: true` + the `YOUTUBE_*` secrets it uploads **unlisted** to YouTube (stdlib `scripts/quest/youtube_upload.py`) and opens one `quest-video`-labeled PR writing the quest's `walkthrough_video:` frontmatter + `.quests/videos.yml` (a label no auto-merge policy matches — human-reviewed). No agent step — deterministic after the engine. OFF behind `QUEST_VIDEO_ENABLED`. See `docs/quests/VIDEO_FRAMEWORK.md`. |
| `quest-fix-loop.yml` | every 2 days; dispatch (gated) | **The FIX lane, on its own cadence.** Ranks not-perfect/not-stuck slices with the same ledger ordering the walk uses, applies the `max_open_quest_prs` / `max_fix_prs_per_run` backpressure, and calls `quest-fix.yml` for each — passing `source_run_id` so it reuses the walk's sealed evidence instead of paying for a second agentic execute pass. Evidence older than `caps.max_evidence_age_days` is skipped, not re-derived at full cost. Split out of the walk because repair is paced by **human review speed**, not by how fast patches can be generated. OFF behind `QUEST_PERFECTION_ENABLED`. |
