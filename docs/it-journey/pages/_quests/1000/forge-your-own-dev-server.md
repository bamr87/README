...
description: 'Turn a bare Debian box into a development server: packages, users, firewall,
...
prerequisites:
  ...
  system_requirements:
  - A spare x86 or ARM machine, a virtual machine, or a cloud instance running Debian
    13
...
rewards:
  badges:
  - 🏆 Forgemaster of the Home Realm - Provisioned a bare machine into a working dev
    server
  - 🔭 Keeper of the Watchtower - Built a live health dashboard for a machine you own
  - 🔬 Instrument Adept - Reads a machine through btop and a toolkit of purpose-built
    TUIs
  ...
  unlocks_features:
  - The on-premises counterpart to cloud provisioning, and the box you will run every
    later quest on
skill_focus: devops
source_file: forge-your-own-dev-server.md
...
- hands-on
- gamified-learning
title: Reach the box and give it a memorable name in your SSH config
validation_criteria:
  completion_requirements:
  - A renamed, firewalled box reachable over SSH with key authentication
  - Language toolchains and a container service stack running and reachable from the
    LAN
  - A health check script and a live dashboard reporting on the box
  knowledge_checks:
  - Understands why a dashboard running as an unprivileged user misreports root-only
    checks
  - Can describe how one tmux session becomes a shared console for the monitor and
    SSH
