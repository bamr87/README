...
description: 'Build an autonomous test→fix→merge loop that perfects its own content:
  Claude Code golems, CI stage gates, sealed evidence, mana budgets, rotating windows,
  coverage ledgers, and auto-merge — reverse-engineered from the real quest-perfection
  engine.'
difficulty: ⚔️ Epic
draft: false
estimated_time: 20-30 hours
excerpt: Forge a loop that walks your content, repairs only what it witnessed break,
  and merges its own fixes behind deterministic gates.
fmContentType: quest
keywords:
  primary:
  - epic_quest
...
- loops
- gamified-learning
title: '1. On a machine logged into Claude Code, mint an OAuth token:'
validation_criteria:
  completion_requirements:
  - All seven chapters completed and the boss fight survived
  - A working miniature loop that tests, fixes, and merges its own content behind
    deterministic gates
  knowledge_checks:
