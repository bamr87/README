---
author: IT-Journey Team
categories:
- Notes
date: '2026-05-17T00:00:00.000Z'
description: Quick reference for the GH-600 exam format, domain weights, passing score,
  and what to expect on exam day.
draft: false
lastmod: '2026-05-17T00:00:00.000Z'
layout: default
permalink: /notes/gh-600/exam-overview/
source_file: exam-overview.md
tags:
- gh-600
- exam
- quick-reference
title: GH-600 Exam Overview
---
# GH-600 Exam Overview

**Official Name:** Developing in Agentic AI Systems  
**Certification:** GitHub Copilot Specialist  
**Passing Score:** 70% (700/1000)  
**Format:** Multiple choice, multi-select, scenario-based

## Domain Weights

| Domain | Topic | Weight |
|---|---|---|
| D1 | Agentic AI in the SDLC | 18% |
| D2 | Tools, environments, and agent capabilities | 18% |
| D3 | Memory, context, and state management | 19% |
| D4 | Evaluating and improving agent performance | 19% |
| D5 | Multi-agent system design | 17% |
| D6 | Responsible autonomy and guardrails | 9% |

**D3 and D4 are the highest-weighted domains. Prioritise them.**

## Study Guide Reference

Official study guide: [Microsoft Learn — GH-600 Study Guide](https://learn.microsoft.com/en-us/credentials/certifications/resources/study-guides/gh-600)

## Key Preparation Milestones

- [ ] Review all 19 sub-skills in the [Skills Measured](/notes/gh-600/skills-measured/) page
- [ ] Complete all 20 quests in the [Agentic Codex arc](/notes/gh-600/)
- [ ] Score ≥ 70% on each domain in the Capstone rubric
- [ ] Review the [Glossary](/notes/gh-600/glossary/) before exam day

---

*Part of: [[GH-600 Agentic AI Quick-Reference Notes]] · Hub: [[The Agentic Codex: GH-600 Study Hub]]*
