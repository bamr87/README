...
session:
  average_scored: 81.5
  engine_cost_usd: 1.8065
  errored: 3
  note: Machine execute-mode evidence exists for only 2 of the 5 planned quests; the
    other 3 did not finish in the engine (the child validator hit its 40-turn ceiling
    mid-build). Those 3 are reasoned-only in this report and carry NO sandbox verdict.
    Evidence consumed as-sealed from ./walk-evidence.json — not re-run, not edited.
  scored: 2
  window: 1 of 6 (offset 5, size 5) of a 26-quest level
source_file: 2026-07-22-digital-artist-0001.md
