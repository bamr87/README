...
quest_count: 5
session: Windowed slice (window 1 of 3, 5 of 12 quests at level 1011). Sealed engine
  evidence consumed as-is (walk-evidence.json / walk-evidence.md); no engine re-run.
  Sandbox = disposable Linux runner; execute mode. Reasoned about the linked journey
  as a Security Specialist learner. Read-only over quest content.
