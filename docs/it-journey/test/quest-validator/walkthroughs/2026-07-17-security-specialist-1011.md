...
session:
  ...
  engine_errored: 1
  engine_scored: 1
  notes: Sealed workflow-minted evidence consumed as-is (walk-plan.json + walk-evidence.json/md).
    Quest 1 (Penetration Testing) produced NO execute evidence — the agentic engine
    timed out after 600s, so that quest is reasoned-only. Quest 2 (Compliance Standards)
    executed fully (4/4 runnable snippets passed).
  quests_walked: 2
