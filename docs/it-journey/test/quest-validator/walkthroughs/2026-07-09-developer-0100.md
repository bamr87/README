...
session:
  ...
  note: Sealed workflow evidence covered only 3 of the 5 planned quests (auth_truncated).
    One quest scored (frontend-docker, 30% fail); two aborted at the engine level
    (max_turns, no verdict); two were never reached. Report coverage is honest about
    this — see §7.
  planned_quests: 5
