...
quest_count: 3
session: Seed walkthrough run in REVIEW mode (read-only, no sandbox execution) by
  the implementing session. Establishes the report contract; the daily routine runs
  in execute mode where commands run for real.
