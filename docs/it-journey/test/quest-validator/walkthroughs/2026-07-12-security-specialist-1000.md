...
session:
  auth_truncated: true
  engine_average: 74.0
  engine_cost_usd: 0.6952
  note: 'Planner selected 4 linked quests; the sealed execute engine scored only the
    first (aws-essentials) before the OAuth budget truncated the run (auth_truncated:
    true, evaluated 1 of 4 requested). Quests 2–4 were walked by static reading only
    and are labelled `reasoned`, never `tested`.'
  total_quests_in_level: 9
  window: 1 of 2 (offset 5, size 5)
source_file: 2026-07-12-security-specialist-1000.md
