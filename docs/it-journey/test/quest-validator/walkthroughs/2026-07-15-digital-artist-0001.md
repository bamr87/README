...
session:
  ...
  note: 'Evidence was pre-computed and sealed by the workflow''s deterministic agentic
    execute-engine step (sandboxed, per-quest verdicts). The quest-walker consumed
    it as-is and did NOT re-run the engine. One quest (Technology Stack Analysis:
    Barodybroject) hit the engine''s max-turns limit and produced NO scored verdict
    — it is reasoned about statically here, never scored. Its "fail" in the counts
    is the engine''s error bucket, not a graded content verdict.'
  scored: 4
