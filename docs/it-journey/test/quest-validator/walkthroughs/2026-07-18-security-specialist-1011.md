...
session:
  ...
  errored: 1
  note: Machine evidence pre-computed and sealed by the quest-walkthrough workflow
    (execute mode, agentic_validate.py). One quest (multi-agent-coordination) timed
    out in the engine at 600s and has NO execution evidence — it is reasoned about
    statically only. Consumed walk-plan.json / walk-evidence.* as-is; no re-run, no
    edits.
  scored: 4
