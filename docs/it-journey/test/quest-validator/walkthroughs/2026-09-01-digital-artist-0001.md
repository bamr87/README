...
session:
  ...
  note: Sealed execute-mode evidence consumed as-is from walk-evidence.json / walk-evidence.md
    (workflow-minted). No engine re-run, no content edits. This window is the first
    5-quest slice (of 26) in the level's sorted order, so a real multi-quest linked-journey
    chain is walkable this session (4 main quests + 1 side quest).
