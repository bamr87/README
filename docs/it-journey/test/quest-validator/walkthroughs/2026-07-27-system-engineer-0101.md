...
session:
  ...
  errored: 1
  evidence_source: sealed walk-evidence.json (agentic execute engine, deterministic
    workflow step)
  notes: Machine evidence sealed by the workflow; the walker consumed it as-is and
    did not re-run the engine. One quest (LaTeX CV) hit the engine's max-turns limit
    and was not scored — it is reasoned about statically only, not tested.
  scored: 4
