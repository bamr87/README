...
description: 'Use when editing the Obsidian vault integration: wiki-links, embeds,
  callouts, backlinks, graph view, the Liquid wiki-index, the JS client resolver,
  or the optional Ruby plugin. Enforces the contract that keeps GitHub Pages and self-build
  deployments rendering identically.'
lastmod: 2026-05-18 12:00:00+00:00
source_file: obsidian.instructions.md
...
| Graph view | [assets/js/obsidian-graph.js](../../assets/js/obsidian-graph.js) | Browser; reads `wiki-index.json` `entries[].outgoing`; cytoscape is **vendored** (`assets/vendor/cytoscape/`, in `vendor-manifest.json`), not a CDN, lazy-loaded via `OBSIDIAN_CONFIG.cytoscapeUrl` |
| Local graph | [assets/js/obsidian-local-graph.js](../../assets/js/obsidian-local-graph.js) | Browser; FAB + panel start `hidden` and are revealed only when the current page is in the index; renders an accessible text-link fallback |
...
2. **Keep the Ruby and JS converters behaviourally equivalent** for: `[[Page]]`, `[[Page|Alias]]`, `[[Page#Heading]]`, `[[Page^block]]` (block-id stripped), `![[image.ext|width]]`, `![[Note Title]]` (transclusion → a Bootstrap card), broken links (a non-navigating `<span class="wiki-link wiki-link-broken">`), inline `#tags` (skipped inside code/links; hex-colour tokens like `#ffffff` are **not** tags), and the full callout type list in `Jekyll::Obsidian::CALLOUT_TYPES`. `[[Page#Heading]]` anchors are slugified by an `anchorize()` helper that mirrors **kramdown's** header-id algorithm (strip leading non-letters → drop chars outside `[a-zA-Z0-9 -]` → spaces to dashes, consecutive kept → downcase); inline `#tag` URLs use Jekyll's default `slugify`. Both helpers exist in **both** layers and must stay byte-identical.
   - **Foldable callouts** (`> [!type]+` / `> [!type]-`) render the title as a `<button class="obsidian-callout-title obsidian-callout-toggle" aria-expanded aria-controls>` with a chevron; the body gets an `id` and (for `-`) the `hidden` attribute. Plain callouts (no marker) render a static `<div role="heading">`. The JS resolver binds **one delegated toggle handler** on the content container in `init()` so it works for callouts rendered by either path — keep that, and keep the markup identical across layers.
