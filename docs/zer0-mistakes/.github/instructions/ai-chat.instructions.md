...
description: Architecture and safety contracts for the AI chat assistant (Claude-
  or Grok-powered widget), the Site Builder session and their chat-proxy — providers
  and translation, auth modes, session credentials, server-side caps, the confirmation-card
  contract, safe rendering, and the dev-only local page/project sandboxes. Read this
  before touching the widget, its JS, or templates/deploy/chat-proxy/.
