...
description: Multilingual translation pipeline — generated-content rules, English
  source of truth, provider contract
lastmod: 2026-07-17 00:00:00+00:00
source_file: translation.instructions.md
title: Multilingual Translation — generated content rules
---
...
8. **A failed page must not discard the pages that succeeded.** `translate.rb` exits 1 when *any* unit fails, so the workflow's normalisation and PR steps gate on `${{ !cancelled() && … }}` — the successful pages still land on the `chore/i18n-translations` PR while the run itself stays red. Do not "simplify" those conditions back to a status-free `if:` (which carries an implicit `success()` and bins ~21 minutes of paid inference per red run), and do not go the other way with `always()`, `continue-on-error`, or a rescue around the failing unit — a cancelled run must push nothing, and a real failure must still be reported. `test_partial_output_survives_failure` in `test/test_i18n.sh` enforces this.
