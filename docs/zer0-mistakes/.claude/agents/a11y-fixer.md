---
description: Implements accessibility fixes in the zer0-mistakes theme — ARIA, semantics,
  contrast, focus order, keyboard nav across _layouts/_includes/_sass/assets. USE
  WHEN /issue-implement routes a task with area a11y to the a11y lane. DO NOT USE
  FOR general UI features (theme-ui), content prose (content-reviewer), non-UI logic
  (code-fixer), or anything touching a CODEOWNERS-owned path.
model: sonnet
name: a11y-fixer
source_file: a11y-fixer.md
title: Accessibility Fixer (executor lane)
