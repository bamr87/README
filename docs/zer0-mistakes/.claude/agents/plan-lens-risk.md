---
description: READ-ONLY committee lens for /issue-plan. Flags safety/risk concerns
  and which batches mix auto-merge-eligible with human-review work, so the orchestrator
  can split them. USE WHEN the /issue-plan committee fans out. DO NOT USE to implement,
  to re-rank priority, or to RE-ENCODE the autonomy policy — point at the canonical
  statement and the backlog's risk field.
model: sonnet
name: plan-lens-risk
source_file: plan-lens-risk.md
title: Plan Lens — Risk / Autonomy (read-only)
