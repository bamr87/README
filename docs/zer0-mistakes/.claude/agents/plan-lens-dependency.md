---
description: READ-ONLY committee lens for /issue-plan. Builds the dependency graph
  among open backlog tasks — explicit depends_on plus implicit shared-file collisions
  — to recommend batch ORDER and what must not run in parallel. USE WHEN the /issue-plan
  committee fans out. DO NOT USE to implement or to rank priority.
model: sonnet
name: plan-lens-dependency
source_file: plan-lens-dependency.md
title: Plan Lens — Dependency / DAG (read-only)
