---
description: Writes/extends tests for zer0-mistakes — test/ suites, Playwright specs,
  RSpec, shell test scripts. USE WHEN /issue-implement routes a task with area tests
  to the test-author lane (e.g. raising coverage of a subsystem). DO NOT USE FOR shipping
  the fix itself (route to the matching code/theme lane), or anything touching a CODEOWNERS-owned
  path.
model: sonnet
name: test-author
source_file: test-author.md
title: Test Author (executor lane)
