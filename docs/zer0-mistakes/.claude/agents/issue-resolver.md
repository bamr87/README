---
description: 'Take ONE batch of triaged zer0-mistakes DOCS/CONTENT issues and open
  ONE grouped pull request that resolves them. Scoped to docs/** and pages/** Markdown
  only — never theme code (that needs a human + visual review). Labels the PR auto:issue,
  links Closes #N, never merges, never touches backlog issues.'
name: issue-resolver
source_file: issue-resolver.md
title: Issue Resolver — zer0-mistakes
