---
description: READ-ONLY committee lens for /issue-plan. Ranks the open backlog tasks
  by priority and impact to recommend WHICH should be sequenced first. USE WHEN the
  /issue-plan committee fans out its lenses. DO NOT USE to implement anything or to
  set risk/autonomy (that's plan-lens-risk + the backlog).
model: sonnet
name: plan-lens-priority
source_file: plan-lens-priority.md
title: Plan Lens — Priority / Impact (read-only)
