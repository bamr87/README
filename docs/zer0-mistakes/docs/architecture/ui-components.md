...
description: Single source of truth for every front-end UI/UX component in the zer0-mistakes
  theme — what it does, the files that implement it (SCSS / includes / layouts / JS
  / plugins), its API surface (classes, data-attributes, JS globals, events, CSS tokens),
  and its automated test coverage. Use it to define, test, and improve the UI.
lastmod: 2026-09-05 00:00:00+00:00
source_file: ui-components.md
tags:
- architecture
- design
- ui
...
- **Capabilities:** `fixed-top` Bootstrap navbar; skip-to-content link (`#main-content`); full-bleed `.container-fluid` bar (capped by `--zer0-navbar-max-width`, default `100%`) with a fluid gutter; responsive lg+ 3-column CSS grid `[brand | nav | utilities]` vs. `<lg` flex, with the brand cluster capped at `30cqi` so it cannot starve the menubar; conditional left-sidebar toggle (omitted when `page.sidebar: false` or no sidebar content); unified-drawer vs. legacy-offcanvas target switching via `site.navigation.unified_mobile_drawer`; conditional `#top-progress-target` nanobar mount when `site.nanobar.position == "navbar"`; container-query name `navbar-main` for progressive degradation.
- **Source:**
...
- **API surface:** ids `#navbar`, `#main-content`, `#top-progress-target`; classes `.navbar-main`, `.navbar-main-start`, `.navbar-brand-group`, `.navbar-home-links`, `.navbar-utility-controls`, `.bottom-shadow`, `.bd-navbar-toggle`, `.navbar-hidden`; container `container-name: navbar-main`; data-attr `data-bs-target` (toggles offcanvas/drawer); CSS vars `--bs-body-bg`, `--zer0-bp-*`, `--zer0-navbar-max-width` (consumer opt-in cap on the bar width)
...
- **Gaps / improvement ideas:** No test asserts the lg+ 3-column grid actually keeps the menubar from painting over Search/Settings (the stated reason for the grid). The nanobar `#top-progress-target` mount path is untested. Body padding from `auto-hide-nav.js` is computed in JS and never asserted, so a regression that drops it (content hidden under the fixed header) would pass CI. *(Menubar label fit, full-bleed bar width, and brand-title survival ARE now covered — see "Navbar — labels and brand cluster" in `features/navbar.spec.js`.)*
...
- **API surface:** classes `.bd-footer`, `.footer-powered-by`, `.footer-powered-by-links`, `.powered-by-link`, `.footer-inline-button`, `.footer-dark-block`, `.footer-nav-columns`, `.footer-latest-posts`, `.footer-subscribe-form`, `.footer-policy-links`; data-attr `data-bs-toggle="offcanvas"`/`"modal"` (`#info-section`, `#cookieSettingsModal`); CSS vars `--zer0-space-*`, `--bs-secondary-color`
- **Element contract:** the two in-page dialog triggers (Info → `#info-section`, Cookie Preferences → `#cookieSettingsModal`) are `<button type="button">`, not anchors — they open UI rather than navigate (WCAG 4.1.2). `.powered-by-link` is therefore element-agnostic and carries the `<button>` reset (`appearance`/`background`/`border`/`color`); `.footer-inline-button` is the same reset for a button sitting in running text. `#backToTopBtn` stays an `<a href="#">` — scrolling to the top is its actual behaviour.
- **Tests:** `test/visual/core/layout-chrome.spec.js` — "powered-by credits are real links" (every credit link's href truthy and not `#`); "footer nav columns use equal width at tablet". `test/visual/core/accessibility.spec.js` — "Accessibility — footer in-page toggles are buttons" pins the element contract above and that both triggers still open their dialog. `test/visual/core/responsive.spec.js` "main landmarks visible at {viewport}" asserts `footer.bd-footer` visible across viewports.
...
### Setup Wizard / Site Builder

- **Purpose:** Nine-step guided wizard (dev-only at `/setup/`, and on the `welcome` layout for unconfigured remote-theme sites) that assembles a complete starter project and, when the local dev proxy is running, hands an embedded Claude session the whole form so it can fill it in, check the machine, read theme source, write the project and start Docker.
- **Capabilities:** Three columns at lg+ (vertical stepper / step panes / sticky side column holding the Claude session on top of the live multi-file preview), stacked below. Steps come from `_data/site_builder.yml` (connect, prereqs, identity, urls, structure, appearance, voice, integrations, build) and carry `.is-done`/`.is-active`/`.is-locked` state — an upcoming step is `disabled` until every earlier step validates; Back is always allowed; the URL hash mirrors the step. Form: brief textarea + identity fields with a 160-char description counter; GitHub/URL fields with a **Suggest** button deriving `url`/`baseurl`; deploy-target radios (remote theme vs gem); site-type quick-pick that preselects collections and rebuilds the menu; eight collection toggles; a navigation row editor; skin cards with gradient swatches and a **Preview on this page** toggle that sets `data-theme-skin` on `<html>`; colour mode, lock, background layers, navbar icon; tone radios + audience select + welcome/about Markdown textareas; integration switches with conditional sub-fields (Giscus IDs, PostHog key); target-folder input + overwrite switch. Generators emit `_config.yml`, `_config_dev.yml`, `Gemfile`, `docker-compose.yml`, `index.md`, `_data/navigation/main.yml`, about page, welcome post, one `pages/<collection>.md` index per collection, `.gitignore`, `zer0.install.yml`, `.env.example`, `README.md`; any file can be overridden (tab shows a dot). Per-file copy/download and a self-extracting bash bundle. Email/URL validate on **blur**; the Build step lists unfilled `data-recommended` fields. Draft persistence: fields, switches, radios, navigation rows, overrides and check results mirrored to `localStorage` under `zer0-setup-draft` (debounced 300ms, flushed on `pagehide`/hidden `visibilitychange`), restored on load; cleared by **Start over** or a successful project write. Prerequisites step: per-OS install/verify commands (macOS default from `navigator.platform`), manual "Done" switches, and — connected — **Run checks** through `POST /api/wizard/check`. Build step: **Check**/**Write project**/**docker compose up|Logs|Stop**/**Is it live?** with a streaming terminal, all proxy-gated (`.sb-needs-proxy`). Claude session: status badge, escape-first Markdown transcript persisted in `sessionStorage` (text turns only), per-step suggested prompts, `.sb-ask` buttons in the form, confirmation cards for every mutation, tools listed in `pages/_docs/features/site-builder.md`.
- **Source:**
  - SCSS: `_sass/components/_setup-wizard.scss`
  - Markup: `_layouts/setup.html`, `pages/setup.html`, `_includes/setup/wizard.html`, `_includes/setup/prereq-checklist.html`, `_includes/setup/claude-session.html`
  - JS: `assets/js/setup-wizard.js` (form, stepper, generators, drafts; exposes `window.Zer0SetupWizard`), `assets/js/site-builder.js` (Claude session, tools, proxy calls)
  - Proxy: `templates/deploy/chat-proxy/dev-proxy.mjs` (`/api/wizard/*`), `templates/deploy/chat-proxy/wizard-store.mjs` (sandbox)
  - Data/config: `_data/site_builder.yml`; `site_builder:` in `_config.yml` (+ endpoint override in `_config_dev.yml`)
- **API surface:** ids `#setup-wizard`, `#wizardTabs`, `#tab-<step>`, `#step-<step>`, `#wizardTabContent`, `#yaml-preview` (shows the ACTIVE file; its `<pre data-file>` names it), `#wizard-file-tabs`, `#btn-download`, `#btn-download-all`, `#btn-copy`/`#btn-copy-full`, `#btn-reset`, `#desc-count`, `#wizard-draft-chip`, `#wizard-review-warnings`, `#cfg-*` (+ `#cfg-*-feedback`), `#col-<collection>`, `#nav-editor`, `#prereq-list`/`#prereq-<id>`/`#btn-run-checks`/`#prereq-summary`, `#connect-status`/`#btn-connect-retry`, `#siteBuilderPanel`/`#sb-*`, `#wizard-terminal`, `#btn-write-project`/`#btn-compose-*`/`#btn-check-live`; classes `.cfg-field`, `.cfg-radio`, `.cfg-switch`, `.cfg-collection`, `.btn-next`/`.btn-prev`, `.wizard-step`, `.wizard-panes`, `.wizard-nav`, `.wizard-side`, `.wizard-cmd` (+ `[data-copy]`/`[data-copy-target]`), `.skin-card`, `.prereq-item[data-state]`, `.sb-needs-proxy`, `.sb-ask[data-ask]`; data-attrs `data-key` (dotted for `posthog.*`, `giscus.*`, `integration.*`), `data-col`, `data-next`/`data-prev`, `data-step-id`, `data-recommended`, `data-file`; JSON blocks `#siteBuilderData`, `#siteBuilderConfig`; storage keys `zer0-setup-draft` (localStorage), `zer0-site-builder-transcript` (sessionStorage); global `window.Zer0SetupWizard` (`getState`, `getSummary`, `schema`, `applyFields`, `getPlan`/`setPlan`/`validatePlan`/`planSchema`/`landingPlan`, `catalog`, `getFiles`/`getFile`, `setOverride`, `getStep`/`setStep`, `setCheckResult`, `setActiveFile`, `buildBundle`, `clearDraft`, `toast`, `on`); DOM events `zer0:wizard:{ready,change,step,files,checks,reset}` on `#setup-wizard`.
- **Stable-height contract:** `#wizardTabContent` is a CSS grid and every `.tab-pane` sits in the same cell (`grid-area: 1 / 1`), so the browser sizes the container to the tallest pane on every reflow; inactive panes stay in layout but are `visibility: hidden` + `pointer-events: none` (which also keeps them out of the a11y tree and the tab order). `.wizard-nav` carries `margin-top: auto`. **Both are required** — the grid stops the container resizing, the auto margin stops the buttons floating up inside a short step. Do not put the panes back on `display: none`. `.wizard-nav` is always `display: flex` and `.wizard-nav .btn` is `white-space: nowrap` so the nav row's own height cannot vary either; measurements in `test/visual/evidence/setup-wizard-nav-alignment/`.
- **Safety contract:** governed by `.github/instructions/ai-chat.instructions.md` §9 — proxy-only (no credential in the page), escape-first rendering, confirmation cards for `set_wizard_fields`/`set_file_override`/`write_site_files`/`run_compose up|down`, and a dev-only sandbox (`wizard-store.mjs`) that allow-lists checks, confines reads to the theme checkout and writes to a new folder under `WIZARD_TARGET_ROOT`.
- **Tests:** `test/visual/features/setup-wizard.spec.js` — nine-step stepper starting on Connect, file preview live at every step with `_config.yml` regenerating on input, Back/Next y-offset stable across steps, blur validation, lock/unlock, Build warnings, draft round-trip through reload and clear on **Start over**, the `pagehide` flush, survival when `localStorage` throws, offline Claude panel + disabled proxy buttons, manual prerequisite toggles, file-tab switching, skin preview toggle. `test/test_wizard_store.mjs` — the sandbox's path, allow-list and overwrite rules.
- **Gaps / improvement ideas:** No end-to-end test drives the Claude loop against a mocked proxy (tool cards, `applyFields` from a model turn). The generated `_config.yml` deserves output assertions per collection toggle. The `welcome` layout still renders the full wizard for GitHub Pages consumers where the proxy can never exist — a lighter "download-only" variant would save bytes.
...
|---|---|---|---|---|
| **critical** | `npm run test:critical` | `.github/actions/playwright-tests` (`project: critical`) in `ci.yml` | Only `@critical`-tagged tests — the user-facing essentials (navigation, search, mobile survival, theming baseline, security) | Platform-independent. **This is the PR-blocking gate.** |
| **smoke** | `npm run test:smoke` | `nightly-extended.yml` (`project: smoke`) | All specs **except** the pixel block (`grepInvert: /homepage visual snapshot/`) — DOM/CSS/layout/a11y behavior on Desktop Chrome | Platform-independent; runs on macOS. Nightly-only since the 2026-07 tiering: failures file a sticky issue instead of blocking PRs. |
