---
agent: agent
description: 'Implement ONE triaged issue/backlog task: route it to the matching specialized
  lane, loop build+test+evidence until green and compatible, document fully, and open
  one PR (auto-merge only for eligible minor classes). Human-dispatched: `/issue-implement
  <#|T-id>`.'
mode: agent
source_file: issue-implement.md
title: Issue Implement
tools:
- run_in_terminal
- read_file
- apply_patch
